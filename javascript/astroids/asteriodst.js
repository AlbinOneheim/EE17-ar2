...
function createAsteroidBelt() {
    ...
    for (var i = 0; i < ROID_NUM + level; i++) {
        ...
        do {
            x = Math.floor(Math.random() * eCanvas.width);
            y = Math.floor(Math.random() * eCanvas.height);
...
function update() {
    ...
    if (textAlpha >= 0) {
        ...
        ctx.fillText(text, eCanvas.width / 2, eCanvas.height * 0.75);
    ...
    ctx.fillText(score, eCanvas.width - SHIP_SIZE / 2, SHIP_SIZE);
    ...
    ctx.fillText("BEST " + scoreHigh, eCanvas.width / 2, SHIP_SIZE);
    ...
    if (ship.x < 0 - ship.r) {
        ship.x = eCanvas.width + ship.r;
    } else if (ship.x > eCanvas.width + ship.r) {
        ship.x = 0 - ship.r;
    }
    if (ship.y < 0 - ship.r) {
        ship.y = eCanvas.height + ship.r;
    } else if (ship.y > eCanvas.height + ship.r) {
        ship.y = 0 - ship.r;
    }
    ...
    for (var i = ship.lasers.length - 1; i >= 0; i--) {
        ...
        if (ship.lasers[i].dist > LASER_DIST * eCanvas.width) {
        ...
        if (ship.lasers[i].x < 0) {
            ship.lasers[i].x = eCanvas.width;
        } else if (ship.lasers[i].x > eCanvas.width) {
            ship.lasers[i].x = 0;
        }
        if (ship.lasers[i].y < 0) {
            ship.lasers[i].y = eCanvas.height;
        } else if (ship.lasers[i].y > eCanvas.height) {
            ship.lasers[i].y = 0;
        }
    }
    ...
    for (var i = 0; i < roids.length; i++) {
        ...
        if (roids[i].x < 0 - roids[i].r) {
            roids[i].x = eCanvas.width + roids[i].r;
        } else if (roids[i].x > eCanvas.width + roids[i].r) {
            roids[i].x = 0 - roids[i].r
        }
        if (roids[i].y < 0 - roids[i].r) {
            roids[i].y = eCanvas.height + roids[i].r;
        } else if (roids[i].y > eCanvas.height + roids[i].r) {
